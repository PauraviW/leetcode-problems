...
# The combination will change each day, and the team running the promotion wants to use a code list to make it
...
codeList = [ [ "apple", "apple" ], [ "banana","anything", "banana" ] ]
shoppingCart = [ "apple", "apple", "banana", "apple", "banana" ]
...
for item in shoppingCart:
    ...
    if temp.isEnd:
        print( 1)
        break
    if item != temp.value and temp.value.strip() != 'anything':
        if groupEnd:
            continue
        else:
            temp = root
    else:
        groupEnd = temp.isgroupEnd
